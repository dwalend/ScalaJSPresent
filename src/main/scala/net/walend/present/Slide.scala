...
object TextLine {
  def apply(text:String,style:Style = Style.HeadLine) = FragmentLine(Seq(TextFragment(text)),style)
}
...
case class FragmentLine(fragments:Seq[Fragment], style: Style = Style.SupportLine) extends Line
...
object Shortcuts {
  ...
  def l2(frags:Fragment*) = FragmentLine(frags,Style.SupportLine)
  ...
  def l3(frags:Fragment*) = FragmentLine(frags,Style.TertiaryLine)
  ...
  def p(frags:Fragment*) = FragmentLine(frags,Style.Plain)
